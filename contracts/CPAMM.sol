...
contract CPAMM is ReentrancyGuard {
    ...
    constructor(address _tokenizedSharesContract) {
        ...
        returns (uint256 tokenOutAmt, uint256 tokenInAmt, uint256 amountOut)
    {
        ...
        tokenOutAmt = tokenizedSharesContract.get(amountOut, _tokenOutPrice);
    }
    ...
    function swap(
        ...
        for (uint256 i = 0; i < userShares.length; i++) {
            ...
            if (tokenIn == share.tokenAddress) {
                uint256 amount = (uint256(tokenInPrice) *
                    uint256(share.buyAmount)) / uint256(share.buyPrice);
        ...
        uint feesAmt = amountIn - amountOut;
